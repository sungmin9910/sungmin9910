...
## 🚀 Quick Start (Jetson Setup)

이 프로젝트는 Jetson Orin Nano의 시스템 환경을 보호하기 위해 가상 환경(`venv`) 사용을 권장합니다.
...
# 가상 환경 접속 (터미널을 켤 때마다 실행)
source ~/st_env/bin/activate
```

### 2. 필수 라이브러리 설치 (가상 환경 내부)
```bash
# Jetson 전용 PyTorch 및 필수 도구 설치
pip install ultralytics flask onnx onnxslim
# (필요 시) Jetson 전용 PyTorch 인덱스 사용
# pip install torch torchvision --index-url https://pypi.jetson-ai-lab.io/jp6/cu126
```

### 3. 시스템 라이브러리 연결 (TensorRT 등)
가상 환경에서 Jetson의 하드웨어 가속 기능을 사용하려면 아래 스크립트를 한 번 실행해야 합니다.
```bash
...
### 4. 메인 대시보드 실행
```bash
python3 SpatialTwin-Orin/edge_jetson/web_spatial_twin.py
```
